...
class _OTPScreenState extends State<OTPScreenPage> {
  ...
  void verifyOtp() async {
    ...
    if (widget.mobileToken == "") {
      ...
      APIProvider()
          .doLogin(
              widget.mobileNumber, Config.isScope == "" ? "" : Config.isScope)
          .then((onValue) {
        if (onValue['flag']) {
          setState(() {
            isLoading = false;
      ...
      if (otpCode.length == 4) {
        ...
        APIProvider()
            .verifyOTP(otpToken as String, int.parse(otpCode))
            .then((onValue) {
          if (onValue['flag']) {
            isLoading = false;
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: <Widget>[
                ...
                ResendTimerScreen(
                    callback: tapOnReSendOtp,
                    isButtonDisable: _isButtonDisabled)
              ],
            ),
          ),
        ),
